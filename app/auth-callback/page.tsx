'use client'

import { useEffect } from 'react'
import { useRouter } from 'next/navigation'
import { CircleNotch } from '@phosphor-icons/react'

export default function AuthCallbackPage() {
  const router = useRouter()

  useEffect(() => {
    async function syncAndRedirect() {
      try {
        // 1. Aqui você faria a chamada real pro seu backend/API
        // Exemplo:
        // const response = await fetch('/api/user/sync', { method: 'POST' })
        // const { hasOrganizations, hasPendingInvites } = await response.json()

        // Simulando a resposta do backend (remova o setTimeout em produção)
        await new Promise(resolve => setTimeout(resolve, 2000))
        const hasOrganizations = false // Simule true ou false aqui
        
        // 2. A Árvore de Decisão
        if (hasOrganizations) {
          // Já faz parte de uma república/produtora
          router.push('/dashboard')
        } else {
          // Conta nova (ou apenas aceitou convite mas não concluiu o setup)
          router.push('/onboarding')
        }

      } catch (error) {
        console.error("Erro ao sincronizar usuário:", error)
        // Se der erro, manda pro onboarding por segurança para ele tentar criar a org de novo
        router.push('/onboarding')
      }
    }

    syncAndRedirect()
  }, [router])

  return (
    <div className="min-h-screen bg-off-white flex flex-col items-center justify-center p-6 text-center">
      <div className="bg-white p-8 rounded-card-lg border border-gray-200 shadow-sm flex flex-col items-center max-w-sm w-full">
        {/* Spinner animado da Reppy */}
        <CircleNotch size={48} weight="bold" className="text-primary-dark animate-spin mb-6" />
        
        <h2 className="font-bricolage text-2xl font-extrabold text-black mb-2 leading-tight">
          Preparando seu acesso
        </h2>
        <p className="font-body text-sm text-gray-500">
          Autenticando com segurança e buscando suas organizações...
        </p>
      </div>
    </div>
  )
}