...
// ─── Modal ───────────────────────────────────────────────────────────────────
const AddCouponModal = ({ onClose, onSave, batches = [] }: AddCouponModalProps) => {
  ...
  const handleTicketTypeToggle = (id: string) => {
    setFormData((prev) => {
      const isSelected = prev.selectedTicketTypes.includes(id);
      return {
        ...prev,
        selectedTicketTypes: isSelected
          ? prev.selectedTicketTypes.filter((tid) => tid !== id)
          : [...prev.selectedTicketTypes, id],
      };
    });
  };
  ...
  const handleSubmit = async (e: React.MouseEvent) => {
    ...
    const payload: CouponPayload = {
      ...
      expires_at:     formData.validUntil
        ? new Date(formData.validUntil).toISOString()
        : null,
      batch_ids:
        formData.applicableTo === "specific" ? formData.selectedTicketTypes : [],
    };
      ...
      <div className="bg-white rounded-[var(--radius-card-md,20px)] border border-[#E0E0D8] shadow-xl w-full max-w-lg overflow-hidden animate-scale-up flex flex-col max-h-[90vh]">
              ...
              {(["all", "specific"] as const).map((val) => (
                <label key={val} className="flex items-center gap-2 cursor-pointer group">
                  <input
                    ...
                    name="applicableTo"
                    value={val}
                    checked={formData.applicableTo === val}
                    onChange={handleChange}
                  ...
                  <span
                    ...
                    style={{ fontFamily: "var(--font-body,'Plus Jakarta Sans',sans-serif)" }}
                  >
                    {val === "all" ? "Todos os ingressos" : "Ingressos específicos"}
                  </span>
                </label>
              ))}
            </div>
              ...
              <div className="relative">
                ...
                >
                  ...
                  <CaretDown
                    size={16}
                    weight="bold"
                    className={`text-[#5C5C52] transition-transform duration-200 ${isDropdownOpen ? "rotate-180" : ""}`}
                  />
                </button>

                {isDropdownOpen && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-[#E0E0D8] rounded-[12px] shadow-lg z-10 max-h-48 overflow-y-auto p-1.5">
                    {batches.length > 0 ? (
                      batches.map((batch) => (
                        <label
                          key={batch.id}
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={formData.selectedTicketTypes.includes(batch.id)}
                            onChange={() => handleTicketTypeToggle(batch.id)}
                      ...
                      <div
                        ...
                        style={{ fontFamily: "var(--font-body,'Plus Jakarta Sans',sans-serif)" }}
                      >
                        Nenhum lote cadastrado neste evento.
                      </div>
                    )}
                  </div>
                )}
        ...
        <div className="px-6 py-4 bg-white border-t border-[#F0F0EB] flex justify-end gap-3 shrink-0">
          <button
            onClick={onClose}
            disabled={isSaving}
            className="px-4 py-2 text-xs font-bold text-[#5C5C52] hover:bg-[#F0F0EB] rounded-[100px] transition-colors disabled:opacity-40"
            style={{ fontFamily: "var(--font-display,'DM Sans',sans-serif)" }}
          >
            Cancelar
          </button>
          <button
            ...
            disabled={isSaving || !formData.code.trim() || !formData.value}
            className="flex items-center gap-2 px-6 py-2 bg-[#0A0A0A] text-[#F7F7F2] text-xs font-bold rounded-[100px] hover:bg-[#222] transition-colors shadow-sm disabled:opacity-40 disabled:cursor-not-allowed"
            style={{ fontFamily: "var(--font-display,'DM Sans',sans-serif)" }}
          >
