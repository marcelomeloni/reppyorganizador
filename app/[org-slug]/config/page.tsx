...
const inputClass = 'w-full px-5 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:border-black focus:bg-white transition-all outline-none font-medium text-sm text-black placeholder:text-gray-300'
        ...
        <main className="p-4 sm:p-6 md:p-10 max-w-3xl mx-auto">
          ...
          <div className="mb-8 md:mb-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-[#9A9A8F] text-sm mt-1.5">Gerencie a identidade da sua organização.</p>
            </div>
            ...
            <button
              ...
              disabled={isLoading}
              className={`
                self-start sm:self-auto flex items-center gap-2 px-5 md:px-6 py-3 rounded-[100px] font-bold text-sm transition-all
                shadow-lg shadow-black/10 active:scale-95 disabled:opacity-60 whitespace-nowrap
                ${saved ? 'bg-[#1BFF11] text-[#0A0A0A]' : 'bg-[#0A0A0A] text-white hover:bg-[#1BFF11] hover:text-[#0A0A0A]'}
              `}
            >
              {isLoading ? <span className="animate-pulse">Salvando…</span>
                : saved   ? <><Check size={18} weight="bold" /> Salvo!</>
          ...
          <form onSubmit={handleSave} className="space-y-5 md:space-y-6 pb-20">
            ...
            <section className="bg-white border border-[#E0E0D8] rounded-[20px] md:rounded-[24px] overflow-hidden shadow-sm">
              <div className="px-4 md:px-6 py-4 border-b border-[#F0F0EB] flex items-center justify-between">
                <span className="text-[11px] font-bold uppercase tracking-wider text-[#0A0A0A]">Aparência Visual</span>
                <span className="text-[10px] font-bold text-[#9A9A8F] bg-[#F0F0EB] px-2.5 py-1 rounded-full">OPCIONAL</span>
              </div>
              <div className="relative">
                <div className="h-32 md:h-40 bg-[#F0F0EB] relative group overflow-hidden">
                  {headerPreview
                    ? <img src={headerPreview} className="w-full h-full object-cover" alt="Capa" />
                    : <div className="w-full h-full flex items-center justify-center"><ImageIcon size={28} className="text-[#E0E0D8]" /></div>}
                  <label className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center cursor-pointer text-white gap-2 backdrop-blur-[2px]">
                    ...
                    <input type="file" hidden accept="image/*" onChange={(e) => { const f = e.target.files?.[0]; if (f) setHeaderPreview(URL.createObjectURL(f)) }} />
                  </label>
                </div>
                <div className="absolute left-4 md:left-6 -bottom-9 md:-bottom-10">
                  <div className="relative group w-20 h-20 md:w-24 md:h-24 rounded-[16px] md:rounded-[20px] border-[4px] md:border-[5px] border-white bg-[#F0F0EB] shadow-xl overflow-hidden">
                    {profilePreview
                      ? <img src={profilePreview} className="w-full h-full object-cover" alt="Perfil" />
                      : <div className="w-full h-full flex items-center justify-center text-[#9A9A8F] font-display font-extrabold text-2xl">{formData.name.charAt(0)}</div>}
                    <label className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center cursor-pointer text-white backdrop-blur-sm">
                      ...
                      <input type="file" hidden accept="image/*" onChange={(e) => { const f = e.target.files?.[0]; if (f) setProfilePreview(URL.createObjectURL(f)) }} />
                    </label>
                  </div>
                </div>
              </div>
            ...
            <section className="bg-white border border-[#E0E0D8] rounded-[20px] md:rounded-[24px] p-4 md:p-6 shadow-sm space-y-5">
              <span className="text-[11px] font-bold uppercase tracking-wider text-[#0A0A0A] block border-b border-[#F0F0EB] pb-4">Informações Gerais</span>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
                ...
                <Field label="Slug (URL)">
                  <div className="relative flex items-center">
                    <span className="absolute left-4 text-[#9A9A8F] text-sm font-medium pointer-events-none">reppy.app/</span>
                    <input required type="text" value={formData.slug} onChange={(e) => setFormData({ ...formData, slug: e.target.value })} className={`${inputClass} pl-[90px] font-mono font-bold`} />
                  </div>
                </Field>
              </div>
              <Field label="Descrição / Bio">
                <textarea rows={3} value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Ex: República focada em integrar bixos e veteranos…" className={`${inputClass} resize-none leading-relaxed`} />
              </Field>
              <div className="space-y-2">
                <span className="text-[11px] font-bold uppercase tracking-wider text-[#9A9A8F] ml-1 block">Categoria</span>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 md:gap-2.5">
                  {CATEGORIES.map(({ value, label, Icon }) => {
                    const active = formData.category === value
                    return (
                      <button key={value} type="button" onClick={() => setFormData({ ...formData, category: value })}
                        className={`flex items-center gap-2.5 md:gap-3 px-3 md:px-4 py-2.5 md:py-3 rounded-xl border-2 transition-all text-sm font-semibold text-left ${active ? 'border-[#0A0A0A] bg-[#0A0A0A] text-white shadow-md' : 'border-[#E0E0D8] bg-white text-[#5C5C52] hover:border-[#0A0A0A]/30 hover:bg-[#F0F0EB]'}`}>
                        <span className={`w-7 h-7 md:w-8 md:h-8 rounded-lg flex items-center justify-center shrink-0 ${active ? 'bg-[#1BFF11]' : 'bg-[#F0F0EB]'}`}>
                          <Icon size={15} weight="bold" className={active ? 'text-[#0A0A0A]' : 'text-[#9A9A8F]'} />
                        </span>
            ...
            {/* Contato */}
            <section className="bg-white border border-[#E0E0D8] rounded-[20px] md:rounded-[24px] p-4 md:p-6 shadow-sm space-y-5">
              <span className="text-[11px] font-bold uppercase tracking-wider text-[#0A0A0A] block border-b border-[#F0F0EB] pb-4">Canais de Contato</span>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
                ...
                <Field label="WhatsApp *" icon={<WhatsappLogo size={12} weight="fill" />}>
                  <input required type="text" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: handlePhoneMask(e.target.value) })} className={`${inputClass} font-mono font-bold`} />
                </Field>
                <Field label="Instagram" icon={<InstagramLogo size={12} weight="fill" />}>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#9A9A8F] font-bold text-sm pointer-events-none">@</span>
                    <input type="text" value={formData.instagram} onChange={(e) => setFormData({ ...formData, instagram: e.target.value })} className={`${inputClass} pl-9`} />
                  </div>
                </Field>
                <Field label="Site oficial" icon={<Globe size={12} weight="fill" />}>
                  <input type="text" value={formData.website} placeholder="https://" onChange={(e) => setFormData({ ...formData, website: e.target.value })} className={inputClass} />
                </Field>
              </div>
            </section>
            ...
            <section className="bg-red-50/40 border border-red-100 rounded-[20px] md:rounded-[24px] p-4 md:p-6 mt-4 group transition-all hover:bg-red-50/70">
              <div className="flex items-start gap-3 md:gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-display font-extrabold text-base md:text-lg text-[#0A0A0A] tracking-tight">Zona de Perigo</h3>
