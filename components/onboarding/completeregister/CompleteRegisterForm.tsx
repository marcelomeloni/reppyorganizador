...
interface CompleteRegisterFormProps {
  ...
  setHasCpfRegistered: (val: boolean) => void
  userCpf: string
}

export function CompleteRegisterForm({ onBack, hasCpfRegistered, setHasCpfRegistered, userCpf }: CompleteRegisterFormProps) {
  ...
  return (
    <div className="bg-white p-8 rounded-[32px] shadow-2xl border border-gray-100 animate-in fade-in slide-in-from-left-6 duration-300 max-w-lg mx-auto">
      <button onClick={onBack} className="mb-6 flex items-center gap-2 text-gray-400 hover:text-black transition-colors font-body text-sm font-bold">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="font-display text-xs font-bold text-gray-400 uppercase ml-1">Nascimento</label>
                <input type="date" className="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-[18px] focus:outline-none focus:border-black transition-all" />
              </div>
              <div className="space-y-1">
        ...
        <div className="text-center space-y-6">
          ...
          <div>
            ...
            <p className="font-body text-sm text-gray-500 mt-2 leading-relaxed">
              Sua conta já está vinculada ao CPF:
            </p>
            {/* Botão de Copiar CPF */}
            <button 
            ...
            >
              <strong className="text-black text-lg font-mono">{userCpf}</strong>
              {copied ? <CheckCircle size={18} className="text-green-600" weight="fill" /> : <Copy size={18} className="text-gray-400 group-hover:text-black" />}
            </button>
            {copied && <span className="text-[10px] text-green-600 font-bold uppercase mt-1 block animate-pulse">Copiado!</span>}
          </div>
          ...
          <button onClick={() => router.push('/dashboard')} className="w-full py-4 bg-black text-white font-display font-bold rounded-[18px] hover:bg-gray-900 transition-all">
            Ir para o Dashboard
          </button>
        </div>
      )}
